    ...
    fetch(`${window.location.origin}/job/${id}`, {
      ...
      .then((data) => {
        if (data.acknowledged === true) {
          ...
          // Fetch updated jobs after deletion
          setIsLoading(true);
        } else {
  ...
  return (
    <div className="max-w-screen-2xl container mx-auto x1:px-24 px-4">
      <div className="my-jobs-container">
        <h1 className="text-white text-center p-4">All My Jobs</h1>
        <div className="search-box p-2 text-center mb-2 flex justify-center items-center md:flex-row flex-col md:gap-4 gap-4">
          <input
            ...
            name="search"
            id="search"
            placeholder=" Search the preferred job"
            className="placeholder:text-ly bg-transparent py-2 p1-3 border focus:outline-none lg:w-6/12 mb-4 w-full"
          />
          <button
            className="bg-blue text-dk font-semibold px-8 py-2 rounded-sm mb-4"
      ...
      <section className="py-1 bg-blueGray-50">
            ...
            <div className="rounded-t mb-0 px-4 py-3 border-0">
              <div className="flex flex-wrap items-center">
                ...
                <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
                  <Link to="/post-job">
                    <button className="bg-ly text-dk active:bg-ly text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">
                      Post a New Job
                    </button>
                  </Link>
                </div>
        ...
        <div className="flex justify-center text-block space-x-8">
          {
            currentPage > 1 && (
